...
def _set_llm_request(
    ...
    if should_send_prompts():
        for i, msg in enumerate(prompts):
            ...
            span.set_attribute("chat.prompt", msg)
        ...
        for message in messages:
            for msg in message:
                ...
                if isinstance(msg.content, str):
                    ...
                    chat_prompt_parts.append(msg.content)
                else:
                    content_str = json.dumps(msg.content, cls=CustomJsonEncode)
                    span.set_attribute(
                        f"{SpanAttributes.LLM_PROMPTS}.{i}.content",
                        content_str,
                    )
                    chat_prompt_parts.append(content_str)
                i += 1
        if chat_prompt_parts:
            span.set_attribute("chat.prompt", "\n".join(chat_prompt_parts))
    ...
    for generations in response.generations:
        for generation in generations:
            ...
            if hasattr(generation, "text"):
                ...
                if first_response:
                    span.set_attribute("chat.response", generation.text)
                ...
                if generation.message.content is str:
                    ...
                    if first_response:
                        span.set_attribute("chat.response", generation.message.content)
                        first_response = False
                else:
                    content_str = json.dumps(generation.message.content, cls=CustomJsonEncode)
                    span.set_attribute(
                        f"{prefix}.content",
                        content_str,
                    )
                    if first_response:
                        span.set_attribute("chat.response", content_str)
                        first_response = False
